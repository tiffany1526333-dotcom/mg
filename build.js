...
async function build() {
  ...
  console.log(`Building… API_BASE=${apiBase || '(relative/same-origin)'}  version=${version}`);
  ...
  // Service worker: stamp the build version so each deploy triggers an update.
  const sw = fs.readFileSync(SW, 'utf8').replace(/__BUILD_VERSION__/g, version);
